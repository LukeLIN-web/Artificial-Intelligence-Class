...
def update_q_table(q_table, old_state, current_action, next_state, r, alpha):
    ...
    q_table[old_state[0], old_state[1], old_state[2], current_action] = (1 - alpha) * q_table[
        old_state[0], old_state[1], old_state[2], current_action] + alpha * (r + next_max_value)
    return q_table
...
def get_state(pipe_sprites, Bird):
    ...
    for pipe in pipe_sprites:
        dist_to_pipe_horz = pipe.rect.left - Bird.rect.left
    ...
    if velocity < 2:  # very slow
        ...
        velocity_category = 5  # fast

    if dist_to_pipe_bottom < 0:  # very close or less than 0
        height_category = 0
...
def reward(Bird, pipe_sprites):
    ...
    if not Bird.is_dead:
        ...
        if dist_to_pipe_bottom <= -40:
            ...
            r = -1
        elif dist_to_pipe_bottom <= 100:
            r = 0
        elif dist_to_pipe_bottom <= 135:
            r = 1
        elif dist_to_pipe_bottom <= 175:
            r = 0
        elif dist_to_pipe_bottom <= 200:
            r = -1
        elif dist_to_pipe_bottom <= 240:
            r = -2
        else:
            r = -5
    # 死了直接扣1000
...
def move_pipe(bird, pipe_sprites, is_add_pipe, score):
    ...
    for pipe in pipe_sprites:
        ...
        if pipe.rect.centerx < bird.rect.centerx and not pipe.used_for_score:
            ...
            score += 0.5
        # 增加新的pipe
        if 10 > pipe.rect.left > 0 and is_add_pipe:
            pipe_pos = Pipe.generate_pipe_position()
    ...
    while True:
        ...
        if not is_start:
            ...
            if is_game_running:
                ...
                # 运行1000次保存一下
                if episodes % 5 == 0 :
                    np.save("./data/{}_{}.npy".format(score, episodes), agent.qtable)
                    # pygame.quit() #在服务器上运行
                    # sys.exit(0)
                # print('Episodes: %s, Current score: %s, Max score: %s, Average score: %s, total = %s'
                #       % (episodes, score, best, average, total))
