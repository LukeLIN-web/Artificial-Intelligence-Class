...
def astar_search_for_puzzle_problem(init_state, dst_state):
    ...
    pq = PriorityQueue()
    """
    pq is a priority queue, the item is the index of state in the open_list
    and the priority is -(g+h)
    """
    curr_idx = 0
    curr_state = start_state
    open_list.append(start_state)
    pq.push(0,-1000) #start state has the lowest priority
    ...
    while len(open_list) > 0:
        ...
        for child_state in childs:
            ...
            pq.push(len(open_list)-1,-(child_state.g + child_state.h))

def expand_state(curr_state):
    """
    Expand current state and generate child state
    :param father_state
    :return: states that can be reached from current state by only one move
    """
    child_state = []
    for moves in Move:
        #don't expand the previous state
        if  curr_state.pre_move == None or ((moves.value + curr_state.pre_move.value) % 4 != 1):
            valid, next_state = once_move(curr_state,moves)
            if valid:
                child_state.append(next_state)
            
    return child_state
    

"""
when we don't use priority queue
we can search the best node manually
"""
#def find_front_node(open_list):
#    """
#    Find the best node from current open_list
#    """
#    front_node = open_list[0]
#    temp_index = 0
#    index = 0
#    
#    for state in open_list:
#        if (state.h + state.g) < (front_node.h + front_node.g):
#            front_node = state
#            index = temp_index
#        temp_index += 1
#        
#    return index, front_node
    

def get_path(curr_state):
    """
    Trace back from the final state to the starting state and returns the move-list
    """
    move_list = []
    while(curr_state.pre_move != None):
        move_list.insert(0,curr_state.pre_move)
        curr_state = curr_state.pre_state
        
    return move_list

def state_in_list(child_state, check_state):
    """
    Check whether the state is in the check_list
    """
    for states in check_state:
        if (child_state.state == states.state).all():
...
def update_cost(curr_state, dst_state):
    """
    Calculate the heuristic functionand update the cost of the state
    """
    curr_state.h = 0
    (rows,cols) = curr_state.state.shape
    
    for i in range(rows):
        for j in range(cols):
            if curr_state.state[i][j] == -1:
                continue
            (dst_row, dst_col) = dst_state.num_pos(curr_state.state[i][j])
            curr_state.h += abs(dst_row-i) + abs(dst_col-j)
            # take Manhattan Distance as heuristic function
